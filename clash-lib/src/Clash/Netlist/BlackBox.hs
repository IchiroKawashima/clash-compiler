      ...
      \case
        ...
        P.Primitive pNm _ _
          ...
          | pNm == "Clash.Explicit.SimIO.pureSimIO#" -> do
              ...
              resM <- resBndr True dst
              case resM of
                Just (_,dstNm,dstDecl) -> case expr of
                  Noop ->
                    return (Noop,decls)
                  Identifier v Nothing | v == dstNm ->
                    return (Noop,decls)
    ...
    (_,[],[],_,[],binders0,Just result) -> do
      ...
      let bindersE = case res0 of
                        ...
                        _ -> case dst of
                          ...
                          _ -> binders2
      ds0 <- concat <$> mapM (uncurry (mkDeclarations' Sequential)) bindersE
      (expr,ds1) <- case res0 of
                      Letrec {} -> mkExpr False Concurrent dst (C.Var result)
                      _ -> case dst of
                        CoreId {} -> pure (Noop,[])
      ...
      (exprInit,ds2) <- mkExpr False Sequential sDst mealyInit
      ...
      (exprArg,ds3)  <- mkExpr False Concurrent iDst mealyIn
      ...
      (clkExpr,clkDecls) <- mkExpr False Concurrent
                                   (NetlistId "__MEALY_CLK__" (termType tcm clk))
                                   clk
      let assn = case expr of
                   Noop -> []
                   _ -> [Seq [AlwaysComb [SeqDecl (Assignment dstNm expr)]]]
